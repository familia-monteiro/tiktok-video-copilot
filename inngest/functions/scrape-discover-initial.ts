...
export const scrapeDiscoverInitial = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const proxyConfig = await step.run('get-proxy-config', async () => {
      ...
      console.log('[Inngest] Computed proxyConfig:', { ...proxyObj, password: proxyObj.password ? '***' : null })
      return proxyObj
    })
      ...
      const jobId = await step.run(`call-scraper-${batchNumber}`, async () => {
        ...
        const payload = {
          ...
          checkpoint: {
            ...
            known_ids: Array.from(allKnownIds)
          },
          callback_url: callbackUrl,
          proxy: proxyConfig,
        }

        console.log('[Inngest] Calling VPS worker with payload:', {
          job_id: id,
          influencer_id,
          tiktok_handle: influencer.tiktok_handle,
          mode: 'initial',
          has_proxy: !!proxyConfig
        })
